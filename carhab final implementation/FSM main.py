...
def execute_instruction(instruction, amount, pivot_amount, centerdepthlimit):
    """
...
def draw_centered_crosshair(frame, cx, cy, pivot_amount):
...
try:
    while True:
        ...
        instruction, amount, pivot_amount, new_state, new_state_time, new_mod_state, new_mod_state_time, executed_id, obstacle_counter = direct_car.interpret_sign(tracker, pivot_amount, frame_width, frame_height, sign_min_depth, 
                                                                                                                                   current_state, state_time, current_mod_state, mod_state_time, executed_id, min_depth, obstacle_counter)
        #draw_centered_crosshair(color_image, 0, 0, pivot_amount)
        # execute the instruction that you just got from the driving logic state machine
        execute_instruction(instruction, amount, pivot_amount, depth_limit)
        rover.cameraPivot(pivot_amount)
        rover.update_camera()
        draw_centered_crosshair(color_image, 0, 0, pivot_amount)
